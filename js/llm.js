...
从用户提供的文档内容中，提取并整理所有会议信息。

重要规则：
1. 文档中若出现 Speaker 1、Speaker 2、与会者A 等占位符，请根据「项目成员列表」和上下文内容，
   推断并替换为对应的真实姓名和部门；若无法确认则保留原始标识
2. 行动项务必指明负责人（从成员列表匹配），并包含截止时间（如文档有提及）
3. 日期格式统一为 YYYY-MM-DD；无法判断时返回空字符串
4. 保留关键决策、重要数据、结论；纪要可以是 Markdown 格式
5. 若某字段无对应内容，返回空字符串或空数组
